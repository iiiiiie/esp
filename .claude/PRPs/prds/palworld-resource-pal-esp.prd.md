...
- [x] Lucky/闪光使用 `IsRarePal()` 独立分类；Alpha/Boss 使用 `GetCharacterID()` + `GetDatabaseCharacterParameter()` + `GetIsBoss()` 独立分类，两者不合并。
- [ ] 首批资源适配器的确切顺序是什么？
...
| 3 | Pal Filter MVP | 实现帕鲁种类、属性、等级、性别、稀有、词条、IV、图鉴筛选 | in progress | with 4 | 2 | - |
...
- **Progress**: 2026-07-18 已实现等级、距离、性别、独立 Lucky 和独立 Alpha/Boss 三态筛选；Lucky 与 Boss 的 Steam 单机样本验证待维护者完成。
