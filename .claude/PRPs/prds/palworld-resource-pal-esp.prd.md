...
| Must | `Shift+T` 打开自有面板 | 提供统一、可扩展的复杂筛选入口 |
...
| 4 | UI & I18n | 实现 Shift+T 面板、中文默认、Language 切换和配置持久化 | pending | with 3 | 2 | - |
...
| 配置 UI | 自有 Shift+T 面板 | 强制依赖 Mod Config Menu | 复杂筛选需要专用界面，减少强制依赖 |
