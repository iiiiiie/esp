...
- Lua to Blueprint compatibility event: `PalworldResourceESP_RemoveTarget` (not used by lifecycle synchronization)
...
`SetTarget` appends one already accepted wild Pal plus its normalized snapshot level and rounded snapshot distance. The overlay keeps target, the `IndividualParameter` used to build metadata, level, distance, Blueprint-normalized gender, Lucky state, Boss state, capture count, CharacterID, element mask, HP IV, attack IV, defense IV, Blueprint nickname, localized passive-skill text, and delimiter-safe passive IDs in sixteen same-order arrays. Every lifecycle synchronization uses one complete generation: `ClearTarget` empties all sixteen arrays, then ordered `SetTarget` calls append the current nearest N records. Runtime lifecycle handling never calls the compatibility `RemoveTarget` event. `ResetSession` removes the overlay before a new world session. The overlay uses one `OnPaint` pass to filter and project every accepted target up to `MAX_DISPLAY_TARGETS`. Before drawing, it compares the Actor's current `IndividualParameter` object and CharacterID with the aligned cached identity. An unavailable or mismatched binding fails closed for that frame and never mutates arrays while `ForEachTarget` is executing; the next coalesced Lua rebuild repairs the complete generation. Visible distance is recalculated from live player and target positions during painting. Name, level, distance, IVs, passive skills, and the top guide can be shown independently. Level, distance, and IV field labels are Chinese by default and switch as one language with the panel. UE 5.1's Widget `DrawText` has no outline input, so each visible label uses four 1px black offset passes followed by the white foreground pass.
...
Dead targets are filtered at both lifecycle speeds: Lua rejects `PalCharacterParameterComponent:IsDead()` during snapshot rebuild, and the Widget calls `PalUtility::IsDead` before drawing each frame. Death, capture, and tracked EndPlay hooks request one coalesced atomic rebuild; they never remove from parallel Blueprint arrays individually.
...
- Keep Cooked Pak files out of Git. Distribute the complete Lua + Pak layout through GitHub Releases, Nexus Mods, or Steam Workshop.
