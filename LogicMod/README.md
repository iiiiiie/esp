...
- Lua to Blueprint: `PalworldResourceESP_RemoveTarget`
...
Dead targets are filtered at both lifecycle speeds: Lua rejects `PalCharacterParameterComponent:IsDead()` during snapshot rebuild, and the Widget calls `PalUtility::IsDead` before drawing each frame. Death, capture, and EndPlay hooks remove one aligned target immediately; full snapshots remain the repair path for broader query or world transitions.
