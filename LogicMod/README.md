...
- Blueprint event to Lua: `PalworldResourceESP_LuaBridgeReady`
...
Blueprint receives only a `PalMonsterCharacter` already accepted by the Lua acquisition boundary. It must not scan actors, enumerate players, or accept a player object from another path.
