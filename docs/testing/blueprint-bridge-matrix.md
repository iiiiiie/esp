...
| Current deployed panel pak | `4F6B2DAEA6EA8ED474CDD3069CD12411D887D8BAC3CEDC9460FC5704DF849E5E`; bilingual passive fallbacks, corrected rarity groups, and right-click exclusion; runtime regression pending |
...
| BP-30 | Passive catalog and tooltips | Catalog categories come from game rank/lottery data and hover text uses localized `SkillDesc` rich text with exact-ID fallback for missing/malformed rows | Pending | Each valid catalog ID resolves its localized name directly; 27 reported effects have bilingual fallbacks, including `Noukin` and `Legend`; numeric plus `Status_Up` tags render through `DT_ESPRichTextStyle`; Legend precedes Rainbow and rank 0/1 reaches Normal. Seven groups compile; 16-file/0-DLL package checks pass. |
