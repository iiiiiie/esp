...
| BP-30 | Passive catalog and tooltips | Catalog categories come from game rank/lottery data and hover text uses localized `SkillDesc` rich text with exact-ID fallback for missing/malformed rows | Pending | Each valid catalog ID resolves its localized name directly; 25 reported effects have bilingual fallbacks; numeric plus `Status_Up` tags render through `DT_ESPRichTextStyle`; Legend precedes Rainbow and rank 0/1 reaches Normal. Seven groups compile; 16-file/0-DLL package checks pass. |
