...
| Current deployed panel pak | `7D6C1F8F2A4EEB2B0918458559FB566362367BB5B8DB76840061DD77555B3065`; restored after the cross-asset rich-text candidate crashed during Login-map startup |
...
| Current Lua script hash | `B9FC6500D97D0B89437E4C33747147D9D2AF5518447C5BCC98D16834985078D6` |
...
| BP-12 | Panel toggle and input | `Shift+Y` opens/closes the panel; A-Z text entry cannot trigger game shortcuts; mouse cursor, UI input, and gameplay input restore correctly | Pending | The 2026-07-20 genuine Workshop install completed two open/close cycles with matching request, dispatch, and `status=ok` completion markers and no panel failure. Text-entry isolation, shortcut suppression, and cursor/gameplay restoration were not retested. |
...
| BP-36 | Panel gameplay-input isolation | While the panel is open, gameplay shortcuts do not open Palworld UI or change cursor ownership; search fields still accept text and editing keys | Pending | `UIOnlyEx` is reinforced by a reversible Mod-owned `PalPlayerController::SetDisableInputFlag`; panel key-down/up return `Handled` after focused children. Panel/ModActor status 3 and package checks pass; Steam checks for Escape, Backspace, T, A-Z, search typing, close, and restored gameplay input remain. |
