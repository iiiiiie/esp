...
| BP-12 | Panel toggle and input | `Shift+Y` opens/closes the panel; A-Z text entry cannot trigger game shortcuts; mouse cursor, UI input, and gameplay input restore correctly | Pending | Current graph combines `UIOnlyEx` with controller `DisableInput` while open and calls `EnableInput` before restoring `GameOnly`; compile/static contracts pass and Steam regression remains. |
