...
| EC-21 | Passive-skill display and filtering | Display uses localized names; no selection shows all; multiple selected skills use AND; hover shows localized effects | Pending | Catalog labels now resolve each ID through `GetPassiveSkillName`, selected/unselected foregrounds have explicit contrast, and tooltip `{EffectValue1..4}` placeholders use the current game row values. Blueprint compile, clean Cook, and 12-file/0-DLL package checks pass; Steam visual verification remains. |
