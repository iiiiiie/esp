...
- Implementation: wrapper-safe snapshots, live target distance, independent metadata visibility, numeric panel ranges, gender, Lucky, Boss, multi-select element filtering, and display-only IV providers, plus capture tooling complete; persistence and runtime regression pending
...
| LogicMod pak SHA-256 | `1EBE5D4B0B2A1477D4077AE7927C236830E50C44AB8E52B72E92D28992CEB93C`; display-only IV checkpoint |
| `main.lua` SHA-256 | `2FCC47E6DBADE8E793BC4CFEB14B9ECE269BABC3EE6FB09AB0B7126F9319F2CE` |
| `config.lua` SHA-256 | `60159901F056F7586419EF6C7DE7887285B2E8F3B1E576A28CE50BCCF6BDD3B3` |
| `user_settings.lua` SHA-256 | `010FF4BFE53971F7CEE79D002A434BDC6EC44D09A57E6FC0E1D7DA736D37FB1D` |
...
| AT-21 | LogicMod package | Generated package contains exactly five `.uasset` and five `.uexp` files with no runtime DLL | Pass | Clean Cook completed 408/408 with 0 errors; UnrealPak lists 10 files, 0 DLL, SHA-256 `1EBE5D4B0B2A1477D4077AE7927C236830E50C44AB8E52B72E92D28992CEB93C` |
...
| AT-28 | Versioned local settings | Only whitelisted bounded scalars round-trip; invalid lines fail closed; the last valid snapshot wins | Pass | Unit suite covers strict `v1/v2/v3/v4/v5` migration, clamping, append format, debug-source and `package.searchpath` resolution, and coalesced runtime writes; the runtime stub omits the path override and resolves beside `config.lua` |
...
8. Enable IV display near ordinary Pals. Confirm each visible label contains plausible `0..100` HP/ATK/DEF values, disable and re-enable the label, then close/reopen the panel to confirm the toggle state.
9. Capture one guided Pal and kill another. Confirm both guides disappear and stay absent after at least six seconds.
10. Change at least three visible settings, wait two seconds, return to Title, and exit. Restart once and confirm the settings restore while capture remains stopped.
11. Extract `USER_SETTINGS_PATH`, `USER_SETTINGS_SAVED`, `USER_SETTINGS_LOADED`, `ADAPTER_REGISTERED`, `PLAYER_AUDIT`, `ENTITY_SNAPSHOT`, `FILTER_RESULT`, `DISPLAY_BUDGET`, `SCAN_DONE`, `DISPLAY_STYLE`, bridge, lifecycle, and error rows.
12. Re-run the privacy/capability scans and compare the crash directory timestamp.

## Completion Rule

ADR-0005 remains `Proposed` until EC-01 through EC-19 pass. After that evidence is recorded, change the ADR to `Accepted`, mark Phase 2 complete in the PRD, update source/deployment hashes, and make the Draft PR ready for review. Multiplayer remains community-pending.
