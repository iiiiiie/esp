...
| AT-21 | LogicMod package | Generated package contains exactly eight `.uasset` and eight `.uexp` files with no runtime DLL | Pass | Clean Cook produced 16 files with no Cook/Blueprint errors; UnrealPak lists 16 files, 0 DLL, SHA-256 `1DF309A397362F597F771FE14A3C227D10BA766966D923D27031ADF27B672CCC` |
...
| AT-38 | Tabbed filter panel | 1180x680 panel uses three tabs, expandable passive groups, and working clear-passive/clear-all actions | Pass | C++ plugin compiles; repeated Blueprint generation compiles Panel status 3 with 1451 nodes; clean Cook and package checks pass |
...
| EC-21 | Passive-skill display and filtering | Display uses localized names; no selection shows all; multiple selected skills use AND; search narrows names; hover renders localized rich-text effects | Pending | Dynamic labels inherit the cooked UMG font; invalid/`None` rows are rejected; red/blue numeric tags use a project-owned rich-text style table. Blueprint compile, clean Cook, and 16-file/0-DLL package checks pass; Steam visual verification remains. |
