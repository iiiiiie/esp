...
- Passive skills are grouped by game rank and lottery metadata into Rainbow, Legend/exclusive, Gold, Normal, and three negative tiers. Dynamic catalog text inherits the cooked UMG font. Empty, `None`, and missing-data rows fail closed before widget creation. A committed localized-name search rebuilds the catalog only on Enter, focus loss, or an explicit Search action. Hover uses a dedicated rich-text tooltip with project-owned styles for confirmed numeric tags. Clear-passive and clear-all actions update both the Blueprint-owned filter state and visible controls.
