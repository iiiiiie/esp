...
Persist panel preferences in `PalworldResourceESP/Scripts/user-settings.log` as an append-only sequence of complete versioned snapshots. New writes use `v9`; the reader accepts strict complete `v1` through `v9` records.

- Every snapshot contains a fixed whitelist of booleans, bounded integers, and two bounded passive-ID strings. The scalar fields cover runtime/profile/preset, language, level endpoints, maximum distance, visible target limit, top guide, name/level/distance/IV/passive visibility, legacy IV minimum, HP/attack/defense IV minima, gender filter, Lucky filter, Boss filter, nine element toggles, and eight passive-category expansion states.
...
- A valid `v1` snapshot migrates in memory by defaulting Lucky and Boss to `all`; `v2` defaults Boss to `all`; `v1`, `v2`, and `v3` default all element toggles to unselected; `v1` through `v4` default IV display to disabled; `v1` through `v5` default the legacy IV minimum to zero; and `v1` through `v6` default passive-skill display to disabled. For `v1` through `v7`, the legacy IV minimum migrates into all three dimensional minima. `v1` through `v8` default passive selections to empty and all passive categories to collapsed. Old records are never rewritten in place. The next stable user change appends a complete `v9` snapshot.
