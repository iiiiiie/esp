...
The first Entity Core implementation proves the contract with Lua-safe fields such as level and reconciliation-time distance. Gender is the first validated Blueprint field provider: Blueprint maps `EPalGenderType` to `0 = unknown`, `1 = male`, or `2 = female` for each already-admitted target and applies the selected filter before projection. Lucky is a separate Blueprint provider backed by `UPalIndividualCharacterParameter::IsRarePal()`: `-1 = unknown`, `0 = normal`, and `1 = Lucky`. Alpha/Boss is another independent Blueprint provider: it reads `GetCharacterID()` from the admitted target, resolves `UPalDatabaseCharacterParameter` through `UPalUtility::GetDatabaseCharacterParameter()`, and calls `GetIsBoss()` to produce `-1 = unknown`, `0 = normal`, or `1 = Boss`. Lucky never implies Boss. Both three-state selectors reject unknown values in either restricted mode. Elements use a fourth Blueprint provider: it calls `HasElementType()` for the nine supported `EPalElementType` values and stores one per-target bit mask (`Normal=1`, `Fire=2`, `Water=4`, `Leaf=8`, `Electricity=16`, `Ice=32`, `Earth=64`, `Dark=128`, `Dragon=256`). No selected bit means all elements; multiple bits use match-any semantics; an unknown target mask is `-1` and fails closed when filtering is active.

IV display and filtering use a fifth typed Blueprint provider. It calls `GetSaveParameter()`, breaks `FPalIndividualCharacterSaveParameter`, and reads `Talent_HP`, `Talent_Shot`, and `Talent_Defense` as `0..100` integers. Provider failure stores `-1` for all affected per-target entries; enhancement ranks are never substituted. HP, attack, and defense each have an independent `0..100` minimum, disabled at zero. Every dimension must be known and greater than or equal to its own minimum before projection.

Passive-skill display and filtering use a sixth typed Blueprint provider. It calls `UPalIndividualCharacterParameter::GetPassiveSkillList()` only for an already-admitted target, resolves every ID through `UPalUIUtility::GetPassiveSkillName()`, and stores both the localized display string and a delimiter-safe ID string in same-order arrays. The panel builds its selectable catalog from `GetSortedPassiveSkillNameArray`, `GetPassiveSkillManager`, and `GetSkillData`; localized `SkillDesc` text is the hover tooltip, with the exact-ID fallback contract recorded in ADR-0004.

Required passive IDs remain a Blueprint-owned `TArray<FName>` capped at four entries. Excluded passive IDs use a separate Blueprint-owned `TArray<FName>` without that cap. Left-click inclusion and right-click exclusion are mutually exclusive for one ID: including removes the same exclusion, and excluding removes the same inclusion. No required selection accepts all Pals; multiple required IDs use AND, so every required ID must occur. Any occurrence of any excluded ID rejects the target after the Lua player gate and before projection. Clear-passive and clear-all operations empty both arrays. Lua sees only the scalar passive-filter revision and never receives either array, enums, save struct, character ID, database object, provider results, or a field-derived Actor reference. Species and capture count remain explicit `bridge` or `unavailable` values until dedicated adapters are validated.
...
- [x] Implement HP/attack/defense IV display and three independent minimum thresholds without marshalling the save struct through Lua.
- [x] Validate displayed `Talent_HP`, `Talent_Shot`, and `Talent_Defense` values with ordinary Steam single-player samples.
- [ ] Validate IV minimum `0`, equality, one-below-threshold rejection, panel restoration, and lifecycle cleanup in Steam single-player.
- [x] Implement localized passive-skill display and Blueprint-owned multi-select AND filtering without marshalling the `TArray<FName>` through Lua.
