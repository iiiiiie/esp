...
IV display and filtering use a fifth typed Blueprint provider. It calls `GetSaveParameter()`, breaks `FPalIndividualCharacterSaveParameter`, and reads `Talent_HP`, `Talent_Shot`, and `Talent_Defense` as `0..100` integers. Provider failure stores `-1` for all affected per-target entries; enhancement ranks are never substituted. The scalar `0..100` minimum is disabled at zero. When positive, all three values must be known and greater than or equal to the minimum before projection.

Passive-skill display uses a sixth typed Blueprint provider. It calls `UPalIndividualCharacterParameter::GetPassiveSkillList()` only for an already-admitted target and resolves every ID through `UPalUIUtility::GetPassiveSkillName()`, preserving the game's active language. The overlay stores only the resulting display string. Lua reads only scalar selectors and visibility toggles; it never receives the passive-skill array, enums, save struct, character ID, database object, provider results, or a field-derived Actor reference. Species, passive-skill filtering, and capture count remain explicit `bridge` or `unavailable` values until their dedicated adapters are validated.
...
- [x] Implement HP/attack/defense IV display and an all-three minimum threshold without marshalling the save struct through Lua.
...
- [x] Implement localized passive-skill display without marshalling the `TArray<FName>` through Lua.
